...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	// 确保查到的Session启用了presence
	SessionSearch->QuerySettings.Set(SEARCH_PRESENCE, true, EOnlineComparisonOp::Equals);

	const ULocalPlayer *LocalPlayer = GetWorld()->GetFirstLocalPlayerFromController();
	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("Calling FindSessions")));
...
{
	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("FindSessionsComplete")));
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	}
...
{
	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Blue, FString::Printf(TEXT("JoinSessionComplete")));
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	}
